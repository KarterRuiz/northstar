...
import {
  CLASS_DATA_CENTER_CHECK_IN_LIMIT,
  classDataCenterStudentInterventionsHref,
} from "./constants";
import { loadClassDataCenterContext } from "./load-class-data-center-context";
import { loadClassDataCenterRecentActivity } from "./load-class-data-center-recent-activity";
...
export type ClassDataCenterCheckInStudent = {
  studentId: string;
  displayName: string;
...
export type ClassDataCenterOverviewData =
  | {
      ok: true;
      role: Role;
      classId: string;
      ...
      checkInCount: number;
      checkIn: ClassDataCenterCheckInStudent[];
      reportCardsLabel: string;
      isCurrentYear: boolean;
      recentActivity: { id: string; summary: string; occurredAt: string }[];
    }
  | { ok: false; message: string };

export const loadClassDataCenterOverview = cache(
  async (classId: string): Promise<ClassDataCenterOverviewData> => {
    ...
    const { role, studentCount, isCurrentYear, schoolYearLabel } = ctx.context;
    ...
    if (enrollmentsRes.error) {
      logServerError("class-data-center-overview.loadEnrollments", enrollmentsRes.error.message);
      return { ok: false, message: GENERIC_INFORMATION_LOAD_ERROR };
    }
    ...
    const [attendanceRes, assignRes, activityRes] = await Promise.all([
      supabase
      ...
      supabase
        ...
        .select("id")
        .eq("class_id", classId),
      loadClassDataCenterRecentActivity(classId, role),
    ]);

    if (attendanceRes.error) {
      logServerError("class-data-center-overview.loadAttendance", attendanceRes.error.message);
    }
    ...
    const assignmentIds = (assignRes.data ?? []).map((row) => row.id);
    const assignmentCount = assignmentIds.length;

    let scoredCellCount = 0;
    if (assignmentIds.length > 0) {
      const { count, error: scoreError } = await supabase
        .from("gradebook_scores")
        .select("id", { count: "exact", head: true })
        .in("assignment_id", assignmentIds.slice(0, 120))
        .neq("status", "missing");
      if (scoreError) {
        logServerError("class-data-center-overview.loadScores", scoreError.message);
      } else {
        scoredCellCount = count ?? 0;
      }
    }
    ...
    if (studentIds.length === 0) {
      return {
        ...
        academicsLabel: academicsPulseLabel({ assignmentCount, scoredCellCount }),
        checkInCount: 0,
        checkIn: [],
    ...
    if (termsRes.error) {
      logServerError("class-data-center-overview.loadTerms", termsRes.error.message);
    }
    ...
    const ranked = rankCheckInStudents(candidates, CLASS_DATA_CENTER_CHECK_IN_LIMIT);
    const nameById = await loadStudentNames(
      supabase,
      ranked.map((row) => row.studentId),
    );
    const checkIn: ClassDataCenterCheckInStudent[] = ranked.map((row) => ({
...
async function loadReportCardFiles(
  ...
  for (const part of chunkIds(studentIds)) {
    ...
    if (error) {
      logServerError("class-data-center-overview.reportCards", error.message);
      continue;
    }
...
async function loadStudentNames(
  ...
  if (error) {
    logServerError("class-data-center-overview.studentNames", error.message);
    return names;
  }
