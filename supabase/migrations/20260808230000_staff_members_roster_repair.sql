-- -----------------------------------------------------------------------------
-- Repair / complete staff_members roster after 20260808220000 failure or partial apply.
--
-- Prior failure modes this migration completes safely:
-- 1) Invitation backfill set profile_id = accepted_user_id when no profiles row exists
--    (accepted_user_id → auth.users; profile may not exist yet).
-- 2) Junction backfills copied class_teachers.class_id / pending_class_ids (and grade
--    equivalents) without joining public.classes / public.grade_levels, so orphan
--    assignment ids violated staff_member_classes_class_id_fkey (and grade FKs).
--
-- This migration is idempotent: safe if 20260808220000 rolled back, failed mid-backfill,
-- partially applied, or already succeeded. It re-runs schema + safe junction backfills.
-- -----------------------------------------------------------------------------

-- 1) Ensure staff_members exists with nullable profile_id FK (null = not yet activated).
...
-- Defensive: if an older/partial definition marked profile_id NOT NULL, open it.
alter table public.staff_members
  alter column profile_id drop not null;

-- Ensure FK only enforces non-null profile_id values (recreate if missing/wrong).
do $$
begin
  if exists (
    select 1
    from pg_constraint
    where conname = 'staff_members_profile_id_fkey'
      and conrelid = 'public.staff_members'::regclass
  ) then
    alter table public.staff_members
      drop constraint staff_members_profile_id_fkey;
  end if;
end $$;

-- Clear any orphan profile_ids before re-adding FK (only possible if FK was absent).
update public.staff_members sm
set profile_id = null
where sm.profile_id is not null
  and not exists (
    select 1 from public.profiles p where p.id = sm.profile_id
  );
...
comment on column public.staff_members.profile_id is
  'Nullable until invitation accept. Set only when a real profiles row exists; never invent ids.';
...
-- 2) Pre-activation junction tables.
...
-- 3) Invitation roster linkage + lifecycle timestamps.
...
-- 4) Safe backfills — only set profile_id when a real profiles row exists.

-- Existing profiles → roster rows (active staff).
...
-- Invitations → roster rows. Attach profile_id only via JOIN to profiles (never orphan FK).
insert into public.staff_members (
  first_name,
  last_name,
  full_name,
...
-- Attach profile_id on existing roster rows when a real profile now matches by accepted_user_id or email.
update public.staff_members sm
set profile_id = p.id
from public.staff_invitations i
...
where sm.profile_id is null
  ...
  and not exists (
    ...
    where other.profile_id = p.id
      and other.id is distinct from sm.id
  );
...
where sm.profile_id is null
  ...
  and lower(trim(sm.email)) = lower(trim(p.email))
  and not exists (
    select 1 from public.staff_members other
    where other.profile_id = p.id
      and other.id is distinct from sm.id
...
-- Link invitations to roster by real profile_id or email.
...
-- Copy pending grade/class ids onto staff_member junctions for linked invites.
...
-- Completes the class/grade backfill even if 20260808220000 stopped here.
insert into public.staff_member_grade_levels (staff_member_id, grade_level_id)
select sm.id, gl.id
from public.staff_members sm
